...
(function () {
	...
	var fs		= require('fs'),
		...
		bower		= {
			...
			knockout	: ['knockoutjs/dist/knockout.js'],
			require		: ['requirejs/require.js', 'requirejs-text/text.js'],
	...
	gulp.task('clean_src', function () {
		return clean(['less/font.awesome/**/*', 'js/modules/**/*', 'fonts/**/*', 'less/bootstrap/**/*'], { cwd: bases.src, read: false }, util.log('CLEANUP:  Asset files removed from ' + chalk.cyan('\'./frontend/src/\'')));
	});
	...
	// COPY FILES - CUSTOM BOOTSTRAP JS
	gulp.task('copy_custom_bootstrap_js', function () {
		return gulp.src('bootstrap/js/*.js', { cwd: bases.custom })
			.pipe(gulp.dest(bases.src + 'js/modules/bootstrap/'));
	});
	// COPY FILES - CUSTOM BOOTSTRAP LESS
	gulp.task('copy_custom_bootstrap_less', function () {
		return gulp.src('bootstrap/less/*.less', { cwd: bases.custom })
			.pipe(gulp.dest(bases.src + 'less/bootstrap/'));
	});
	// COPY FILES - KNOCKOUT
	...
	// COPY CUSTOM KNOCKOUT
	gulp.task('copy_custom_knockout', function () {
		return gulp.src('knockout/*.js', { cwd: bases.custom })
			.pipe(gulp.dest(bases.src + 'js/modules/knockout/'));
	});
	...
	gulp.task('copy_assets', ['copy_jquery', 'copy_velocity', 'copy_knockout', 'copy_require', 'copy_crossroads', 'copy_hasher', 'copy_signals', 'copy_fontawesome_fonts', 'copy_fontawesome_less', 'copy_custom_bootstrap_js', 'copy_custom_bootstrap_less', 'copy_custom_knockout', 'copy_projectdocs'], function () {
	...
	// COMPILE - JSON - KNOCKOUT MODEL DATA
	gulp.task('build_json', function () {
		return gulp.src(['json/**/*.json'], { cwd: bases.src })
			.pipe(gulp.dest(bases.dev + 'js/'));
	});
	...
	gulp.task('build_html', ['process_jade_index', 'process_jade_error', 'process_jade_pages', 'process_jade_components', 'process_md'], function () {
	...
	// WRAPPER - BUILD
	gulp.task('build', ['build_images', 'build_html', 'build_srv_config', 'build_css', 'build_js', 'build_json'], function (cb) {
		cb();
	...
	gulp.task('watch', function (cb) {
		...
		gulp.watch(['json/**/*.json'], { cwd: bases.src }, ['build_json']);
